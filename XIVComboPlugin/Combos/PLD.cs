namespace XIVComboExpandedPlugin.Combos
{
        ...
        protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
        {
            if (actionID == PLD.GoringBlade)
            {
                ...
                if (comboTime > 0)
                {
                    ...
                    if (lastComboMove == PLD.RiotBlade && level >= PLD.Levels.GoringBlade && DebuffDuration(PLD.Debuffs.GoringBlade) < 8)
    ...
    internal class PaladinoGCDFeature : CustomCombo
    {
        protected override CustomComboPreset Preset => CustomComboPreset.PaladinoGCDFeature;

        protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
        {
            if (actionID == PLD.SpiritsWithin)
            {
                if (level >= PLD.Levels.CircleOfScorn && !GetCooldown(PLD.CircleOfScorn).IsCooldown)
